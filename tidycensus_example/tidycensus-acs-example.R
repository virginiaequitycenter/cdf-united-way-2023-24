####################################################
# Tidycensus Example - ACS County level data for the Charlottesville Regional Equity Atlas
####################################################
# Acquire ACS data
...
ccode <- read_csv("tidycensus_example/county_codes.csv")
...
# Get Data
# pull tables (easier to just pull these tables separately)
...
# ....................................................
# 3. Metrics specific to locality level - Household Income by race  ----
...
county_data <- county_data %>% 
  ...
  separate(geoid, into = c("state", "locality"), 
           sep = c(2)) %>% 
  select(GEOID, state, locality, NAME, year, totalpopE, totalpopM, whiteE, whiteM, blackE, blackM, asianE, asianM, indigE, indigM, othraceE, othraceM, multiE, multiM, ltnxE, ltnxM, everything())
...
# 6. Save ----
# save to RDS
saveRDS(county_data, file = "data/county_data_example.RDS") 

# or save to csv
write.csv(county_data, "data/county_data_example.csv", row.names=FALSE)
