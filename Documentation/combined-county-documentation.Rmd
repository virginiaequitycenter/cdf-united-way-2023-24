...
### Median Household Income [B19001]
...
When pulling Census data at the county-level using `tidycensus` we can only pull data for individual counties, the package contains no way to aggregate values across a range of counties. In order to do this, we must pull the data in for each county of interest, then manually agggregate the values across the counties.
...
- [socialexplorer.com](https://www.socialexplorer.com/explore-tables): Helps build tables and maps easily
